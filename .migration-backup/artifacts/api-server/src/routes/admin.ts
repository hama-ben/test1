...
router.post("/admin/payments/:paymentId/approve", async (req, res): Promise<void> => {
  ...
  // Extend from the later of: current expiry or now — this is the ONLY place
  // in the whole subscription flow where subscriptionExpiresAt is written.
  // It is always additive on top of whatever the driver already has, never
  // an overwrite. Total bonus = 30 base days + 3 gift days = 33 days.
  const now            = new Date();
  const baseDate       = driver?.subscriptionExpiresAt && driver.subscriptionExpiresAt > now
    ? driver.subscriptionExpiresAt
    : now;
  const TOTAL_BONUS_DAYS = 30 + 3; // 30-day subscription + 3-day gift for an accepted receipt
  const newExpiry      = new Date(baseDate.getTime() + TOTAL_BONUS_DAYS * 24 * 60 * 60 * 1000);
  ...
  // Extend subscription (cumulative — adds on top of the existing balance)
  await db
    .update(usersTable)
  ...
  await insertTargetedAnnouncement(
    payment.driverId,
    "تم قبول دفعتك ✅",
    `تم إضافة ${TOTAL_BONUS_DAYS} يوماً (30 يوم اشتراك + 3 أيام هدية) إلى حسابك. ينتهي اشتراكك في: ${newExpiry.toLocaleDateString("ar-DZ")}.`
  );
